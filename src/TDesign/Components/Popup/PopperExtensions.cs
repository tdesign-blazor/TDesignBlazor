...
{
    public static async ValueTask<PopperInstance> InvokePopupAsync(this IJSRuntime js, ElementReference selectorRef, ElementReference popupRef, PopperOptions options)
    {
        var optionRef = DotNetObjectReference.Create(options);
        var jsObject = await js.ImportScriptAsync();
        var popper = await jsObject.InvokeAsync<IJSObjectReference>("popup.create", selectorRef, popupRef, options, optionRef);
